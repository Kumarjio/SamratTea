   ...
   $(".legal-menu" + $('#transactionmenu').val()).addClass(" collapse in ");
    ...
    $('.datepicker').datepicker({
     ...
     $("#addnewDtlDiv").click(function() {
        ...
        $.ajax({
            url: basepath + "journalvoucher/createDetails",
            type: 'post',
...
$("#journalVoucher").click(function(){
   // alert("");
    if(getVoucherMasterValidation()){
        if(detailvalidation()){
            if(getDebitCreditEqualValidation()){
           var formData = $("#frmJournalVoucher").serialize();
           ...
           $("#journalVoucher").css("display","none");
           $.ajax({
            url: basepath + "journalvoucher/saveData",
            type: 'post',
            data: {formDatas: formData,mode:mode,voucherMasterId:voucherMasterId},
            success: function(data) {
               $("#journalVoucher").css("display","block");
                if(data==1){
                     $( "#save_voucher_detail_data" ).dialog({
                                                ...
                                                Ok: function() {
                                                 ...
                                                 modal: true,
                                                    buttons: {
                                                        Ok: function() {
                                                             window.location.href = basepath + 'journalvoucher';
                                                            $( this ).dialog( "close" );
                                                        }
                                                      }
                                                    });
...
function getTotalDebit(){
 ...
 function deleterecord(id){
			...
			$.ajax({
			type: "POST",
			url:  basepath+"journalvoucher/delete",
			data: {id: id},
			success: function(data)
				{
				window.location.href = basepath+'journalvoucher';
