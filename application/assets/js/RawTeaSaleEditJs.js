...
function getValidation(dtlId) {
    var totalBag = 0;
    var remainingBag = 0;
    var numberofBag = ($("#NumberOfBags_" + dtlId).val() == "" ? 0 : parseInt($("#NumberOfBags_" + dtlId).val()));
    var PrvblendBag = ($("#hdTxtBlended_" + dtlId).val() == "" ? 0 : parseInt($("#hdTxtBlended_" + dtlId).val()));
    var blendBag = ($("#txtused_" + dtlId).val() == "" ? 0 : parseInt($("#txtused_" + dtlId).val()));
    totalBag = numberofBag+PrvblendBag;
    remainingBag =  parseInt(totalBag)-parseInt(blendBag);
  
    //console.log("nu "+numberofBag+":"+blendBag);
    if (blendBag > totalBag) {
        return false;
    } else {
            $("#hdTxtBlended_"+dtlId).val(blendBag);
            $("#NumberOfBags_"+dtlId).val(remainingBag);
            $("#NumberOfBagsTxt_"+dtlId).text(remainingBag);
        return true;
    }
}
function validation() {
