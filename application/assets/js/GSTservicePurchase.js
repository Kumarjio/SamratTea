 ...
 $("#vendor").customselect();
   ...
   $("#saveRawMaterial").click(function(){
      ...
      if(getValidation()){
         ...
         //  var basepath = $("#basepath").val();
			$("#saveRawMaterial").css("pointer-events","none");
           ...
           $.ajax({
            ...
            success: function(data) {
               if(data==1){
				...
				$("#saveRawMaterial").css("pointer-events","");
...
// raw tea sale gst
function delServicePurchase(id,no)
{
    $("#purch_no_info").text(no);
    $("#dialog-confirm-servpurch").dialog({
        resizable: false,
        height: 140,
        modal: true,
        buttons: {
            "Ok": function() {
                ...
                $("#del_Purch_servc").css("display","none");

                $.ajax({
                    type: 'POST',
                    url: basepath + "gstservicepurchase/delservPurchaseGST",
                    data:{"srvPurchID":id,"invoice":no},
                    dataType:'json',
                    contentType: "application/x-www-form-urlencoded; charset=UTF-8", 
                    success: function(data) 
                    {
                        $("#servPurchaseResponse").modal({"backdrop"  : "static",
                              "keyboard"  : true,
                              "show"      : true                    
                        });
                        ...
                        if(data.ERROR==1)
                        {
                            ...
                            if(data.MSG.venAdjustErr.length>0)
                            {
                                 errMsg+="<p> "+data.MSG.venAdjustErr+"</p>";
                            }
                            if(data.MSG.venPaymentErr.length>0)
                            {
                                 errMsg+="<p> "+data.MSG.venPaymentErr+"</p>";
                            }


                            $("#servPurchaseResponseData").html(errMsg );
                            $(".sb_body").css({
                                'background':'#ff5050',
                                'color':'#FFF'
                            });
                            ...
                            $("#del_Purch_servc").css("display","inline-block");
                        }
                        if(data.ERROR==2)
                        {
                            $("#servPurchaseResponseData").text(data.MSG);
                            $(".sb_body").css({
                                'background':'#ff5050',
                                'color':'#FFF'
                            });
                        ...
                        {   
                            ...
                            $('#sp_close_btn').attr("onclick", "location.reload()");
