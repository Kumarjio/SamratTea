...
class doreceivedmodel extends CI_Model{
 ...
 /**
     ...
     */
     public function getStockwithtransporter($transporterId,$compnyId,$warehouseid,$fdate,$tdate){
        
        $whereWarehouse = "";
        $whereDate = "";
        if($warehouseid>0)
        {
            $whereWarehouse = " AND purchase_invoice_detail.`warehouse_id` =".$warehouseid;
        }
        if($fdate!="" && $tdate!="")
        {
            $whereDate = " AND do_to_transporter.transportationdt BETWEEN '".date("Y-m-d",strtotime($fdate))."' AND '".date("Y-m-d",strtotime($tdate))."' ";
        }
            ...
            $sql="SELECT 
                     ...
                     FROM
                        ...
                        WHERE purchase_invoice_master.from_where <> 'SB' AND purchase_invoice_master.company_id=".$compnyId." 
                            AND `do_to_transporter`.`transporterId` =".$transporterId." 
                            ".$whereWarehouse." ".$whereDate."
                            AND(`do_to_transporter`.`chalanNumber`='' OR `do_to_transporter`.`chalanNumber` IS NULL)"
