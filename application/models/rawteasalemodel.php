...
public function getBlendedBag($bagDtlId){
   ...
   private  function updateBillMaster($taxinvoiceId,$searcharray){
    ...
    public function GSTupdateRawTeaSaleMaster($searcharray){
        ...
        $rawTeasalemaster['placeofsupply'] = NULL;
        $rawTeasalemaster['transporter_id'] = $searcharray['transporterid'];
        $rawTeasalemaster['hsn'] = $searcharray['txtHSN'];
        $rawTeasalemaster['state_code'] = $searcharray['txtplaceofsupply'];
...
/*@method insertintoVouchrDtl
    ...
    public function GSTinsertintoRawTeaSaleMaster($vMastId,$searcharray,$rawTeaSaleInvoice){
        ...
        $rawteasale['placeofsupply']=NULL;
        $rawteasale['transporter_id'] = $searcharray['transporterid'];
        $rawteasale['hsn'] = $searcharray['txtHSN'];
        $rawteasale['state_code']= $searcharray['txtplaceofsupply'];
    ...
    public function GSTinsertrawTeaSaleDtl($masterId, $dtlArr) {
        ...
        for ($i = 0; $i < $numberOfDtl; $i++) {
            ...
            $rawteaSaleDtl['delivery_charge'] = ($dtlArr['txtDelivery'][$i] == "" ? 0 : $dtlArr['txtDelivery'][$i]);
   ...
   /* public function getCurrentvatrate($startYear, $endYear) {
    ...
    public function GSTRawTeaSalemasterData($rawteaslMastId){
        $sql = "SELECT 
                ...
                rawteasale_master.hsn,
                rawteasale_master.transporter_id
              FROM
                rawteasale_master 
     ...
     $sql="SELECT 
            ...
            `rawteasale_detail`.delivery_charge,
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                ...
                $data[] = array(
                    ...
                    "saleKgs" => ($rows->qty_of_sale_bag * $rows->num_of_sale_bag),
                    "amt"=>$rows->amt,
                    ...
                    "delivery_charge"=>$rows->delivery_charge,
                    "gstTaxableamount"=>$rows->gstTaxableamount,
                    "HSN"=>$rows->HSN
                );
            }
	...
	*/
	...
	public function RawTeaSaleMaterDataGST($masterId)
	{
		...
		$sql = "SELECT 
				  ...
				  customer.`pin_number`,
				  customer.`telephone`,
				  cus_state.`state_code` AS customerStateCode,
				  cus_state.`state_name` AS customerState,
				...
				FROM
				  ...
				  LEFT JOIN state_master 
					ON rawteasale_master.state_code = state_master.`state_code` 
				  LEFT JOIN state_master AS cus_state
				  ON customer.`state_id` = cus_state.`id`
				WHERE `rawteasale_master`.`id` =".$masterId;
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                $data = array(
					...
					"customerStateCode"=>$rows->customerStateCode,
					"customerState"=>$rows->customerState,
...
/*
@module : Garden Tea Sale OR Raw Tea Sale
@method : deleteRawTeaSaleBillGST
@date : 27.03.2018
@by : Mithilesh
@Table For Del : customerbillmaster,rawteasale_detail,rawteasale_master,voucher_detail,voucher_master
*/

    public function deleteRawTeaSaleBillGST($salebillID,$insertArry)
    {
        $voucherID = $this->getVoucherMasterIDRawTeaSale($salebillID);

        try 
        {
            $this->db->trans_begin();

            // delete customerbillmaster
            $custbillmaster_whr = array(
                "customerbillmaster.invoicemasterid" => $salebillID,
                "customerbillmaster.saletype" => 'R' // R = "Raw Tea Sale OR Garden Tea Sale"
            );

            $this->db->where($custbillmaster_whr);
            $this->db->delete('customerbillmaster'); 
               
            // delete rawteasale_detail
            $this->db->where('rawteasale_detail.rawteasale_master_id', $salebillID);
            $this->db->delete('rawteasale_detail');    

            // delete rawteasale_master
            $this->db->where('rawteasale_master.id', $salebillID);
            $this->db->delete('rawteasale_master');    
               
            // delete voucher_detail
            $this->db->where('voucher_detail.voucher_master_id', $voucherID);
            $this->db->delete('voucher_detail');    
            
            // delete voucher_master
            $this->db->where('voucher_master.id', $voucherID);
    ...
    private function getVoucherMasterIDRawTeaSale($salebillID)
    {
        $voucherID =0;
        $sql = "SELECT rawteasale_master.`voucher_master_id` FROM rawteasale_master WHERE rawteasale_master.`id`=".$salebillID;
        $query = $this->db->query($sql);
        if($query->num_rows()>0)
        {
            $row = $query->row();
            $voucherID = $row->voucher_master_id;
        }
        return $voucherID;
