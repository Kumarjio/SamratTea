...
class gsttaxinvoicemodel extends CI_Model {
    ...
    public function SaleBillMasterPrint($masterId){
        ...
        $sql="SELECT 
				  ...
				  customer.`pin_number`,
				  customer.`telephone`,
				  customer.`state_id` AS custStateID,
				  custState.`state_name` AS customerState,
				  custState.`state_code` AS customerStateCode,
				 ...
				 FROM
					...
					ON sale_bill_master.`state_code` = state_master.`state_code`
					INNER JOIN state_master AS custState
					ON customer.`state_id` = custState.`id`
					LEFT JOIN transport
    ...
    public function getPacketProduct() {
        ...
        $sql = "SELECT `product_packet`.`id` AS productPacketId,
                     ...
                     `packet`.`packet`,
                      ...
                      `product_packet`.`hsn_code` AS hsn_code
 ...
 $sql= "SELECT
            id,  srl_no,  salebillno,GST_placeofsupply,state_code,
     ...
     */
     public function insertintoSaleBillMaster($searcharray,$vMastId,$slno,$salebillno){
           ...
           $saleBillMaster = array();
            ...
            $saleBillMaster["GST_placeofsupply"]=NULL;
            $saleBillMaster["state_code"]=$searcharray["placeofsupply"];
            ...
            $saleBillMaster['duedate'] = ($searcharray['txtDueDate'] == "" ? NULL : date("Y-m-d", strtotime($searcharray['txtDueDate'])));
...
function getCustomerAccId($cus_id,$compny){
  ...
  public function getCreditDays($cusId){
    ...
    public function updateSaleBillDetails($newSaleId,$dtlArr){
        ...
        //discount
            $saleBillDetails['discount_rate'] = ($dtlArr['txtDiscountRate'][$i] == "" ? 0 : $dtlArr['txtDiscountRate'][$i]);
            $saleBillDetails['discount'] = ($dtlArr['txtDiscount'][$i] == "" ? 0 : $dtlArr['txtDiscount'][$i]);
            $saleBillDetails['delivery_charge'] = ($dtlArr['txtDelivery'][$i] == "" ? 0 : $dtlArr['txtDelivery'][$i]); // added on 24.04.2018
   ...
   /* public function UpdateData($saleBillMaster,$searcharray){
	...
	public function get_last_srl_no($cid,$yid)
	{
        ...
        {
            $data=0;
            $sql="select `product_packet`.`sale_rate`,`product_packet`.`net_kgs`,`product_packet`.`hsn_code` from product_packet where id=".$fid."";
...
/*
@method : deleteSaleBillTaxInvoiceGST
@date : 27.03.2018
@by : Mithilesh
@Table For Del : customerbillmaster,sale_bill_details,sale_bill_master,voucher_detail,voucher_master
*/

    public function deleteSaleBillTaxInvoiceGST($salebillID,$insertArry)
    {
        $voucherID = $this->getVoucherMasterIDSaleBillTaxInvoice($salebillID);
        ...
        try 
        {
            ...
            $custbillmaster_whr = array(
                ...
                "customerbillmaster.saletype" => 'T'
            );

            $this->db->where($custbillmaster_whr);
            $this->db->delete('customerbillmaster'); 
               
            // delete sale_bill_details
            $this->db->where('sale_bill_details.salebillmasterid', $salebillID);
            $this->db->delete('sale_bill_details');    

            // delete sale_bill_master
            $this->db->where('sale_bill_master.id', $salebillID);
            $this->db->delete('sale_bill_master');    
    ...
    private function getVoucherMasterIDSaleBillTaxInvoice($salebillID)
    {
        $voucherID =0;
        $sql = "SELECT sale_bill_master.`voucher_master_id` FROM sale_bill_master WHERE sale_bill_master.`id`=".$salebillID;
        $query = $this->db->query($sql);
        if($query->num_rows()>0)
        {
            $row = $query->row();
