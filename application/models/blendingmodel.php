...
class blendingmodel extends CI_Model {
    ...
    public function getTeaStock($garden = "", $invoice = "", $lotnumber = "", $grade = "",$comp) {
        $data = array();
//        $sql = "SELECT 
...
//            WHERE PID.`garden_id`='".$garden."' AND PID.`invoice_number`='".$invoice."' AND PID.`lot`='".$lotnumber."' AND PID.`grade_id`='".$grade."'";
		...
		$sql ="SELECT 
				...
				INNER JOIN `purchase_invoice_master` PIM
						ON PIM.`id` = PID.`purchase_master_id`
                INNER JOIN 
                        `purchase_bag_details` PBD ON PID.`id` =PBD.`purchasedtlid`
                INNER JOIN 
                        do_to_transporter DOT ON PID.`id`= DOT.`purchase_inv_dtlid` AND DOT.`in_Stock`='Y'
                ...
                INNER JOIN `location` ON DOT.`locationId`=`location`.`id`  
                INNER JOIN `teagroup_master` ON PID.`teagroup_master_id` = `teagroup_master`.`id`
             WHERE PID.`garden_id`='".$garden."' AND PID.`invoice_number`='".$invoice."' AND PID.`lot`='".$lotnumber."' AND PID.`grade_id`='".$grade."' AND PIM.company_id=".$comp;

        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                
                $NumberOfStockBag =($rows->actual_bags -  ($this->getBlendedBag($rows->purchaseBagDtlId)+$this->getsaleOutBag($rows->purchaseBagDtlId)+$this->getstockOutBag($rows->purchaseBagDtlId)));
                ...
                if ($NumberOfStockBag != 0) {
                    $data[] = array(
                        ...
                        "Numberofbags" =>$NumberOfStockBag,  //($rows->actual_bags - $this->getBlendedBag($rows->purchaseBagDtlId)),//$rows->NumberOfStockBag,
                        "kgperbag" => $rows->net,
...
public function getBlendedData($blendId){
     $sql="SELECT `blending_details`.`blending_master_id`,
                    ...
                    PID.`id` AS purchaseDtl,
                    PID.`teagroup_master_id`,
                    PID.`invoice_number`,PID.`price`,PID.`cost_of_tea`,
                    PID.`lot`,garden_master.`garden_name`,
                    PID.`garden_id`, grade_master.`grade`,PID.`grade_id`,
                    location.`location`,teagroup_master.`group_code`,
            ...
            FROM
                    ...
                    INNER JOIN `location` ON DOT.`locationId`=`location`.`id`  
                    INNER JOIN `teagroup_master` ON PID.`teagroup_master_id` = `teagroup_master`.`id`
                    INNER JOIN  purchase_bag_details ON purchase_bag_details.id = `blending_details`.`purchasebag_id`
                    INNER JOIN  bagtypemaster ON bagtypemaster.id=purchase_bag_details.`bagtypeid`
                    WHERE `blending_details`.`blending_master_id`='".$blendId."' AND `blending_details`.`qty_of_bag`>0 ORDER BY garden_master.`garden_name`,PID.`invoice_number`, `blending_details`.`id`";
     $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                    
                $stockBag = ($this->getPurchasedBag($rows->purchasebag_id) - ($this->getBlendedBag($rows->purchasebag_id)+$this->getsaleOutBag($rows->purchasebag_id)+$this->getstockOutBag($rows->purchasebag_id)));
                $data[] = array(
                   ...
                   // "pricePerBag"=>$rows->price,
                    ...
                    "blendedCost" =>($rows->number_of_blended_bag * $rows->cost_of_tea * $rows->qty_of_bag),
...
public function getStock() {
    ...
    public function getBlendingMasterData($Id) {
        ...
        $this->db->select('blending_master.id ,
                            ...
                            blending_master.blendedprice,
                            blending_master.blendedBag,
                            blending_master.blendedKgs,
                            blending_master.avgblendCost,
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                $data = array(
                    ...
                    "blendedprice"=>$rows->blendedprice,
                    "blendedBag"=>$rows->blendedBag,
                    "blendedKgs"=>$rows->blendedKgs,
                    "avgblendCost"=>$rows->avgblendCost,
                    "yearId" => $rows->yearid,
    ...
    public function insertBlndDtl($masterId, $dtlArr) {
        ...
        for ($i = 0; $i < $numberOfDtl; $i++) {
            ...
            $blendingDtl['costperkg'] = $dtlArr['hdpriceperbag'][$i];
     ...
     */
    public function getBlendingList($cmpnyId,$yearId) {
        $data = array();
        $sql = "SELECT
                    blending_master.`id`,
                    blending_master.`blending_number`,
               ...
               WHERE blending_master.`companyid`=".$cmpnyId ." AND blending_master.yearid=".$yearId.
                " GROUP BY  blending_master.`id` ORDER BY blending_master.`blending_date`";
    ...
    public function checkDuplicateRefno($refno,$cmpy,$yearId){
        $check = array(
            "blending_ref"=>$refno,
            "companyid"=>$cmpy,
            "yearid"=>$yearId
        );
         $sql = $this->db->select('id')->from('blending_master')->where($check)->get();
          if ($sql->num_rows() > 0) {
