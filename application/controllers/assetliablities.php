...
class assetliablities extends CI_Controller {

    function __construct() {
        parent::__construct();
		
        $this->load->model('balancesheetmodel', '', TRUE);
    ...
    public function index() {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $result='';
            $headercontent='';
            $page = 'asset_liability/header_view';
            $header = '';
  ...
  public function assetliablitypdf()
  {
        ...
        $result['LiablitiesData']= $this->balancesheetmodel->getLiablitiesData($companyId,$yearid,$fiscalStartDt,$toDate,$frmDate);
		$this->db->freeDBResource($this->db->conn_id); 
        $result['AssetData']= $this->balancesheetmodel->getAssetData($companyId,$yearid,$fiscalStartDt,$toDate,$frmDate);
		$this->db->freeDBResource($this->db->conn_id); 
		...
		$result['LiablitiesSum'] =  $this->findGroupBySum($result['LiablitiesData'],'GroupDescription','Liabilities');
		$result['AssetSum'] =  $this->findGroupBySum($result['AssetData'],'GroupDescription','Asset');
		...
		$page = 'asset_liability/balance_sheet_pdf_view.php';
        //  $html = $this->load->view($page, $result);
         $html = $this->load->view($page, $result, TRUE);
                // render the view into HTML
                //$html="Hello";
         $pdf->WriteHTML($html); 
         $output = 'balancesheet' . date('Y_m_d_H_i_s') . '_.pdf'; 
         $pdf->Output("$output", 'I');
         exit();
	...
	/* 	$header.= '</table>';
				...
				$header.='<table width="100%">';
					$header.='<tr><td align="center"><b>Balance Sheet</b><br>';
